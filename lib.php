...
/**
 * This file contains main class for the course format wbsingleactivity
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_wbsingleactivity extends core_courseformat\base {
    ...
    public function extend_course_navigation($navigation, navigation_node $node) {
        ...
        if (has_capability('moodle/course:viewhiddensections', $context)) {
            ...
            if (!empty($modinfo->sections[1])) {
                ...
                $orphanednode = $node->add(get_string('orphaned', 'format_wbsingleactivity'),
     ...
     *
     * WB Singleactivity course format uses one option 'activitytype'
     *
    ...
    public function course_format_options($foreditform = false) {
        ...
        if ($courseformatoptions === false) {
            $config = get_config('format_wbsingleactivity');
        ...
        if ($foreditform && !isset($courseformatoptions['activitytype']['label'])) {
            $courseformatoptionsedit = array(
                'activitytype' => array(
                    'label' => new lang_string('activitytype', 'format_wbsingleactivity'),
                    'help' => 'activitytype',
                    'help_component' => 'format_wbsingleactivity',
                    'element_type' => 'select',
     ...
     *
     * Format WB singleactivity adds a warning when format of the course is about to be changed.
     *
    ...
    public function create_edit_form_elements(&$mform, $forsection = false) {
        ...
        if (!$forsection && ($course = $PAGE->course) && !empty($course->format) &&
                $course->format !== 'site' && $course->format !== 'wbsingleactivity') {
            // This is the existing course in other format, display a warning.
            $element = $mform->addElement('static', '', '',
                    html_writer::tag('span', get_string('warningchangeformat', 'format_wbsingleactivity'),
                        array('class' => 'error')));
        ...
        if ($PAGE == $page && $page->has_set_url() &&
            ...
            if (!$this->get_activitytype()) {
                if (has_capability('moodle/course:update', context_course::instance($this->courseid))) {
                    ...
                    redirect($url, get_string('erroractivitytype', 'format_wbsingleactivity'));
                } else {
