...
namespace format_wbsingleactivity\output;

use core_courseformat\output\section_renderer;

/**
 * Renderer for outputting the WB singleactivity course format.
 *
    ...
    public function display($course, $orphaned) {
        ...
        if ($orphaned) {
            if (!empty($modinfo->sections[1])) {
                $output .= $this->output->heading(get_string('orphaned', 'format_wbsingleactivity'), 3, 'sectionname');
                $output .= $this->output->box(get_string('orphanedwarning', 'format_wbsingleactivity'));
